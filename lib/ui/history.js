...
ChatHistory.prototype.addMessageHelper = function (message) {
  var authorLength = message.getAuthor().length + 3,
      ...
      messageMaxWidth = parentWidth - authorLength - 3,
  ...
  if (messageText.length < messageMaxWidth) {
    ...
    var lines = '',
        isCurrentCharALineBreak = false,
        currentLineCharsCount = 0;

    // Iterate over each character in the message text.
    for (var i = 0; i < messageText.length; i += 1) {
      // Check if the current line is a line-break.
      isCurrentCharALineBreak = messageText.charAt(i) === '\n';

      // Check if the current amount of characters in the line has exceeded
      // the maximum (for the line).
      if (currentLineCharsCount > messageMaxWidth || isCurrentCharALineBreak) {
        // Reset the amount of characters variable.
        currentLineCharsCount = 0;
        // Append a line-break.
        lines += '\n';
        // Append the amount of speaces to matche the indentention.
        lines += Array(authorLength).join(' ');

        // Check if the current line is not a line-break.
        if (!isCurrentCharALineBreak) {
          // Append the current character to the output message text.
          lines += messageText.charAt(i);
        }
      } else {
        // Increase the amount of characters variable.
        currentLineCharsCount += 1;
        // Append the current character to the output message text.
        lines += messageText.charAt(i);
      }
    }
